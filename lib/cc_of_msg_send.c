...
int
cc_send_hello( sw_info* cc_sw_info, buffer *buf ) 
{
  int ret;
  buffer *send_buf;
  int xid;
 ...
 send_buf = cc_create_hello( xid );
 free_buffer(buf);
 
  ret = cc_send_to_secure_channel( send_buf );
  if ( ret == 0 ) {
...
cc_send_echo_reply(sw_info* cc_sw_info,buffer *buf ) 
{
  ...
  send_buf = cc_create_echo_reply( cc_sw_info->xid_latest, buf );
  free_buffer( buf );
...
cc_send_features_request( sw_info* cc_sw_info,buffer* buf )
{
  ...
  send_buf = cc_create_features_request( cc_generate_xid(&(cc_sw_info->xid_latest), cc_sw_info->xid_table_) );
  free_buffer(buf);
...
cc_send_get_config_request(sw_info* cc_sw_info, buffer* buf)
{
	...
	send_buf = cc_create_get_config_request(cc_sw_info->xid_latest);
	return ret = cc_send_to_secure_channel(cc_sw_info, send_buf);
...
cc_send_set_config(sw_info *cc_sw_info, buffer* buf) 
{
 ...
 send_buf = cc_create_set_config( cc_sw_info->xid_latest, cc_sw_info->config_flags,cc_sw_info->miss_send_len);
  free_buffer(buf);
...
cc_send_error_msg( sw_info* cc_sw_info, uint16_t type, uint16_t code, buffer *data ) 
{
  ...
  xid = cc_generate_xid(&(cc_sw_info->xid_latest), cc_sw_info->xid_table_);
  send_buf = cc_create_error(cc_sw_info->xid_latest, type, code, data );
...
int
cc_send_flow_mod(sw_info* cc_sw_info, struct ofp_match match,uint64_t command,uint16_t idle_timeout, uint16_t hard_timeout,
					uint16_t priority, uint32_t buffer_id, uint16_t port, uint16_t flags,uint16_t out_port, openflow_actions *actions)
{
	buffer* send_buf;
	uint32_t xid; 	
	int ret;
...
int
cc_send_packet_out(sw_info* cc_sw_info, uint32_t buffer_id, uint16_t in_port, openflow_actions *actions, buffer* data)
{
	buffer* send_buf;
	uint32_t xid;
	int ret;
...
int
cc_send_port_mod(sw_info* cc_sw_info, 	uint16_t port_no,uint8_t hw_addr[OFP_ETH_ALEN], uint32_t config,
...
int
cc_send_stats_request(sw_info* cc_sw_info,uint16_t type, uint16_t length, uint16_t flags)
{
	buffer* send_buf;
	int ret;
	uint32_t xid;
...
int
cc_send_get_desc_stats(sw_info* cc_sw_info,	uint16_t flags)
{
	buffer* send_buf;
	int ret;
	uint32_t xid;

	send_buf = cc_create_desc_stats_request(cc_sw_info->xid_latest, flags);
	if( send_buf == NULL )
	{
		log_err_for_cc("cc_create_packet_out");
		return CC_ERROR;
...
int
cc_send_flow_stats_request(sw_info* cc_sw_info, uint16_t flags, struct ofp_match match, uint8_t table_id,uint16_t out_port)
{
	buffer* send_buf;
	int ret;
	uint32_t xid;
...
int
cc_send_aggregate_stats_request(sw_info* cc_sw_info, uint16_t flags,struct ofp_match match, uint8_t table_id, uint16_t out_port)
{
	buffer* send_buf;
	int ret;
	uint32_t xid;

	send_buf = cc_create_aggregate_stats_request(cc_sw_info->xid_latest, flags,  match,  table_id,  out_port);
	if( send_buf == NULL )
	{
...
int
cc_send_table_stats_request(sw_info* cc_sw_info, uint16_t flags)
{
	buffer* send_buf;
	int ret;
	uint32_t xid;
...
int
cc_send_port_stats_request(sw_info* cc_sw_info, uint16_t flags, uint16_t port_no)
{
	buffer* send_buf;
	int ret;
	uint32_t xid;
...
int
cc_send_queue_stats_request(sw_info* cc_sw_info, uint16_t flags, uint16_t port_no, uint32_t queue_id)
{
	buffer* send_buf;
	int ret;
	uint32_t xid;
...
int
cc_send_vendor_stats_request(sw_info* cc_sw_info, uint16_t flags, uint32_t vendor, buffer* body)
{
	buffer* send_buf;
	int ret;
	uint32_t xid;
...
cc_send_msg_to_switch(sw_info* cc_sw_info, buffer* buf)
{
	...
	*/
	...
	cc_send_to_secure_channel(cc_sw_info, new_buf);
