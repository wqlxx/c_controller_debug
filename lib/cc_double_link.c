...
create_dlist_with_mutex( pthread_mutex_t *mutex ) {
  private_dlist_element *element = malloc( sizeof( private_dlist_element ) );
...
create_dlist(void) {
  ...
  element->mutex = malloc( sizeof( pthread_mutex_t ) );
...
delete_dlist_element( dlist_element *element ) {
  ...
  free( ( private_dlist_element * ) element );
...
delete_dlist( dlist_element *element ) {
  ...
  for ( e = element; e != NULL; ) {
    ...
    e = e->next;
    free( ( private_dlist_element * ) delete_me );
  }
