...
cc_create_header( const uint32_t xid, const uint8_t type, const uint16_t length ) {
  ...
  struct ofp_header *header = (struct ofp_header*)buf->data;
  if(header== NULL)
  {
...
cc_create_echo_request(uint32_t xid, buffer* buf)
{
	...
	buffer* echo_request = cc_create_header(xid, OFPT_ECHO_REQUEST,  ( uint16_t ) ( sizeof( struct ofp_header ) + data_length ) );
...
cc_create_echo_reply(uint32_t xid,buffer* buf)
{
	...
	if ( data_length > 0 ) {
		memcpy( ( char * ) echo_reply->data + sizeof( struct ofp_header ), buf->data, data_length );
  	}
...
buffer *
cc_create_vendor(  uint32_t transaction_id,  uint32_t vendor,  buffer *data ) {
  void *d;
  uint16_t length;
